...
#define CMDNAME_SETUP_VAR3 ("setup_var3")
...
static grub_err_t
grub_cmd_setup_var (grub_command_t cmd, int argc, char *argv[])
{
    ...
    grub_efi_uintn_t name_size = MAX_VARIABLE_SIZE;

    grub_uint16_t isMode2 = 0;
    grub_uint16_t isMode3 = 0;

    if (0 == grub_strcmp(cmd->name, CMDNAME_SETUP_VAR2))
    {
        isMode2 = 1;
    }
    else if (0 == grub_strcmp(cmd->name, CMDNAME_SETUP_VAR3))
    {
        isMode3 = 1;
    }
    ...
    for(;;)
    {
        ...
        if(((name_size == INSYDE_SETUP_VAR_NSIZE && 0 == grub_memcmp(name, INSYDE_SETUP_VAR, name_size)) ||
            (isMode2 && name_size == INSYDE_CUSTOM_VAR_NSIZE && 0 == grub_memcmp(name, INSYDE_CUSTOM_VAR, name_size))))
        {
            ...
            setup_var_size = INSYDE_SETUP_VAR_SIZE;
            ...
            if(offset > setup_var_size)
            {
                ...
                if(isMode3)
                {
                    grub_printf("Offset over limited size, ignoring.\n\n");
                    continue;
                }
                else
                {
                    grub_printf("Offset over limited size, error.\n\n");
                    status = -1;
                    break;
                }
            }
...
static grub_command_t cmd_setup_var3;
...
GRUB_MOD_INIT(setup_var)
{
    ...
    cmd_setup_var3 = grub_register_command ("setup_var3", grub_cmd_setup_var,
                                             "setup_var3 offset [setval]",
...
GRUB_MOD_FINI(setup_var)
{
    ...
    grub_unregister_command(cmd_setup_var3);
